  ...
  return   (
    ...
    <section className="mb-4">
      ...
      <div className="relative">
        {prev && <button className="w-12 absolute -left-6 z-10 top-1/2 hover:scale-[1.06] -translate-y-1/2  h-12 rounded-full bg-white text-black flex items-center justify-center" 
        onClick={slideFavoritesPrev}><Icon name="prev" size={24} /></button>}
        {next && <button className="w-12 absolute -right-6 z-10 top-1/2 hover:scale-[1.06] -translate-y-1/2 h-12 rounded-full bg-white text-black flex items-center justify-center"
        onClick={slideFavoritesNext}><Icon name="next" size={24} /></button>}
        <ScrollContainer innerRef={favoritesRef}
        className="flex scrollable overflow-x gap-x-6">
          {favoriteCategories.map((category, index) => <WideCategory key={category.index} category={category}/>)}
        </ScrollContainer>
      </div>
    </section>

    <section>
      <Title title="Hepsine Gözat" />
